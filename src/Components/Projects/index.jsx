  ...
  const ProjectsData = [
    {
      ...
      blurb: "The Karen Project currently functions as a clean, interactive Lorem Ipsum generator, providing a modern approach to creating placeholder text. Looking ahead, I plan to expand it with customizable templates and interactive features, transforming it into a versatile tool for generating and refining digital content.",
    },
    {
      name: "Whack-O-Jacko",
      languages: ["/logos/javascript-original.svg",
      ...
      blurb: "A Whack-a-mole style game with a halloween theme. This was a 4 day group project at IO Academy within our first 6 weeks. I was responsible for the start and finish modal design and functionality, I also created all teh artwork for the project.",
    },
    {
      ...
      blurb: "Think 'GuitarHero' with a functional twist. Aimed at keyboard training with a fun gamification interface. This project was a joint venture with a friend and is still underway. Accuracy is rewarded with bonus points and currently has a mode selector - easy, hard and pro.",
    },
    {
      name: "Splitter",
      languages: [
        ...
        {ProjectsData.map((project, index) => (
          <div key={index} className="h-full w-full bg-primary rounded-2xl">
            <div className="w-full h-5/12 rounded-t-xl contain-content ">
              ...
              <img className="scale-250 translate-y-25 translate-40 hover:scale-133 hover:translate-y-3 hover:translate-x-0 transition-all  duration-500 ease-in-out" src={project.image} alt={project.name} />
            </div>
            ...
            {project.languages.map((lang, i) => (
              ...
              src={lang} 
              alt="language logo" 
              className="inline-block h-2/3 mx-4"
              />
              ))}
            </div>
            <div className="mx-3">
              <h3 className="mb-2  text-text-primary opacity-50">{project.name}</h3>
              <p className="text-sm font-extralight  text-text-primary opacity-50">{project.blurb}</p>
            </div>
          </div>
          </div>
          ))}
